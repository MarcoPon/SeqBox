...
Being written in Python 3, SeqBox tools are naturally multi-platform and have been tested successfully on various versions of Windows, on some Linux distros either on x86 or ARM, FreeBSD, and on Android (via QPython). No test was done on OS X but it should works there as well (feedback welcome).   
